    ...
    return (
      <ChartContext.Provider value={{ config }}>
        <div
          ...
          data-chart={chartId}
          className={cn(
            // Enhanced: Added better default typography and grid styling
...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        // Enhanced: Added backdrop-blur for glassmorphism and reduced opacity on background
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              // Enhanced: Added hover transition
              "[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 hover:opacity-80 transition-opacity cursor-default"
            )}
          >
            ...
            <span className="text-sm font-medium">{itemConfig?.label}</span>
          </div>
        );
      })}
    </div>
