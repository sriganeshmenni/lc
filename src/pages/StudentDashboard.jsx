...
import { Loader2, CheckCircle, Clock, Link2, AlertCircle } from 'lucide-react';
import { toast } from 'sonner';

export const StudentDashboard = () => {
  // Authentication context for student user data
  const { user } = useAuth();
  ...
  // Parallel data fetching for dashboard initialization
  // Loads both available placement links and the student's submission history
  ...
  const handleRegister = (link) => {
    ...
    const isRegistered = submissions.some(s => s.linkId === link.id);
    if (isRegistered) {
      toast.info('You have already registered for this link.');
      return;
    }
  ...
  const handleSubmitRegistration = async (submissionData) => {
    try {
      ...
      await loadData(); // Refresh data to show new submission
    } catch (error) {
      throw error;
    }
  };

  // Filter links to show only active ones with future deadlines
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Stats Cards - Overview of student activity */}
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl">
                {Math.max(0, activeLinks.length - completedSubmissions.length)}
              </div>
        ...
        {/* Content Tabs - Available Opportunities and History */}
                ...
                {activeLinks.map((link) => {
                  ...
                  return (
                    <div key={link.id} className="relative">
                      {isRegistered && (
                        <div className="absolute -top-2 -right-2 z-10 bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 shadow-sm">
                          <CheckCircle className="w-3 h-3" /> Registered
                        </div>
                      )}
                      <LinkCard
                        link={link}
                        onRegister={() => handleRegister(link)}
                        // Pass disabled state if already registered
                          ...
                          return (
                            <TableRow key={submission.id}>
                              <TableCell className="font-medium">
                                {link?.title || 'Unknown Opportunity'}
                              </TableCell>
                              <TableCell>
                                {link?.deadline 
      ...
      {/* Registration Proof Upload Dialog */}
